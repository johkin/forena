...
export function ClubDashboard({ organization, sections, team, activity, members, initialInvitations, workspaces, tasks, defaultView, canManageTeam, source }: Props) {
  ...
  const [invitationRole, setInvitationRole] = useState<"leader" | "guardian">("guardian");
  ...
  async function inviteTeamMember(form: HTMLFormElement) {
    ...
    const response = await fetch("/api/team-invitations", {
      ...
      body: JSON.stringify({
        ...
        personDisplayName: data.get("personDisplayName") ? String(data.get("personDisplayName")) : undefined,
      }),
    });
    const result = await response.json();
  ...
  return (
    <main>
      ...
      {showInvitationForm && <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setShowInvitationForm(false); }}><section className="modal" role="dialog" aria-modal="true" aria-labelledby="invitation-form-title"><div className="card-heading"><div><p className="eyebrow">{team.name}</p><h2 id="invitation-form-title">Bjud in till laget</h2></div><button className="icon-button" onClick={() => setShowInvitationForm(false)} aria-label="Stäng" type="button">✕</button></div><form onSubmit={(event) => { event.preventDefault(); void inviteTeamMember(event.currentTarget); }}><label>Roll<select name="role" value={invitationRole} onChange={(event) => setInvitationRole(event.target.value as "leader" | "guardian")}><option value="guardian">Vårdnadshavare</option><option value="leader">Ledare</option></select></label>{invitationRole === "guardian" && <label>Spelarens namn<input name="personDisplayName" required maxLength={120} placeholder="Förnamn och efternamn" /></label>}<label>E-postadress<input name="email" type="email" required autoComplete="email" placeholder="namn@example.se" /></label><p className="form-help">Mottagaren får en personlig inloggningslänk. Länken gäller i sju dagar och kan bara användas av samma e-postadress.</p><div className="modal-actions"><button className="secondary" onClick={() => setShowInvitationForm(false)} type="button">Avbryt</button><button className="primary" disabled={savingInvitation} type="submit">{savingInvitation ? "Skickar…" : "Skicka inbjudan"}</button></div></form></section></div>}
    </main>
  );
}
