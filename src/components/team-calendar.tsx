...
type Props = { activities: Activity[]; timeZone: string };
...
export function TeamCalendar({ activities, timeZone }: Props) {
  ...
  return <section className="card team-calendar" id="calendar" aria-labelledby="calendar-title">
    ...
    <div className="calendar-grid">
      {Array.from({ length: 42 }, (_, index) => {
        ...
        return <div className={`calendar-day ${items.length ? "has-activity" : ""}`} key={key}><strong>{day}</strong>{items.slice(0, 3).map((item) => <span key={item.id} title={item.title}><b>{formatter.format(new Date(item.startsAt))}</b> {item.title}</span>)}{items.length > 3 ? <small>+{items.length - 3} till</small> : null}</div>;
      })}
    </div>
  </section>;
}
