...
export function ActivityEditorModal({mode,organization,team,members,activity,source,onClose,onNotice}:Props) {
  ...
  function prepare(event:FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const invitationAudience=String(data.get("invitationAudience")||"players") as "players"|"leaders";
      ...
      setPayload({teamId:team.id,title:String(data.get("title")),description:String(data.get("description")),location:String(data.get("location")),invitationAudience,invitationSendMinutesBefore,responseDueRule,reminderMinutesBeforeDue,...base,endsOn:recurring?String(data.get("endsOn")):startsOn,weekdays:recurring?data.getAll("weekdays").map(Number):[new Date(`${startsOn}T00:00:00Z`).getUTCDay()||7]});
      setError(undefined);
