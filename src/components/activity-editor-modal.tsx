...
export function ActivityEditorModal({mode,organization,team,members,activity,source,onClose,onNotice}:Props) {
  ...
  const players=members.filter(member=>member.teamRole!=="leader");
  const leaders=members.filter(member=>member.teamRole==="leader");
  ...
  function prepare(event:FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if(personIds.length) occurrences.forEach(item=>invitationScheduleForOccurrence(item.startsAt,timeZone,{invitationSendMinutesBefore,responseDueRule,reminderMinutesBeforeDue}));
      setPreview(occurrences);
      setPayload({teamId:team.id,title:String(data.get("title")),description:String(data.get("description")),location:String(data.get("location")),personIds,invitationSendMinutesBefore,responseDueRule,reminderMinutesBeforeDue,...base,endsOn:recurring?String(data.get("endsOn")):startsOn,weekdays:recurring?data.getAll("weekdays").map(Number):[new Date(`${startsOn}T00:00:00Z`).getUTCDay()||7]});
      setError(undefined);
