...
export function ActivityEditorModal({mode,organization,team,members,activity,source,onClose,onNotice}:Props) {
  ...
  useEffect(()=>{if(mode!=="create"||source!=="database")return;fetch(`/api/team-groups?teamId=${encodeURIComponent(team.id)}`).then(r=>r.ok?r.json():{groups:[]}).then(body=>setGroups(body.groups??[])).catch(()=>setGroups([]));},[mode,source,team.id]);
  ...
  function prepare(event:FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const invitationSendMinutesBefore=Number(data.get("invitationSendMinutesBefore"));
      const responseDueRule=String(data.get("responseDueRule")) as ResponseDueRule;
      const reminderMinutesBeforeDue=Number(data.get("reminderMinutesBeforeDue"));
      ...
      if(invitationAudience==="group"&&!invitationGroupId) throw new Error("Välj en undergrupp.");
      occurrences.forEach(item=>invitationScheduleForOccurrence(item.startsAt,timeZone,{invitationSendMinutesBefore,responseDueRule,reminderMinutesBeforeDue}));
      setPreview(occurrences);
      setPayload({teamId:team.id,title:String(data.get("title")),description:String(data.get("description")),location:String(data.get("location")),invitationAudience,invitationGroupId,invitationSendMinutesBefore,responseDueRule,reminderMinutesBeforeDue,...base,endsOn:recurring?String(data.get("endsOn")):startsOn,weekdays:recurring?data.getAll("weekdays").map(Number):[new Date(`${startsOn}T00:00:00Z`).getUTCDay()||7]});
      setError(undefined);
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)onClose();}}>
    <section className="modal activity-editor-modal" role="dialog" aria-modal="true" aria-labelledby="activity-editor-title">
      ...
      <form onSubmit={prepare} onChange={()=>{setPreview(undefined);setPayload(undefined);}}>
        ...
        {mode==="create"?<><fieldset><legend>Målgrupp för kallelsen</legend><label>Målgrupp<select name="invitationAudience" value={audience} onChange={event=>setAudience(event.target.value as "players"|"leaders"|"group")}><option value="players">Alla spelare i laget</option><option value="leaders">Alla ledare i laget</option>{groups.length?<option value="group">Undergrupp</option>:null}</select></label>{audience==="group"?<label>Undergrupp<select name="invitationGroupId" required defaultValue=""><option value="" disabled>Välj undergrupp</option>{groups.map(group=><option key={group.id} value={group.id}>{group.name}</option>)}</select></label>:null}<p className="member-group-label">Målgruppen bestäms när kallelsen skickas, så ändringar i laget fram till dess följer med automatiskt.</p></fieldset><fieldset className="invitation-schedule"><legend>Kallelse och svarstid</legend><div className="form-row"><label>Skicka kallelsen<select name="invitationSendMinutesBefore" defaultValue="10080"><option value="20160">14 dagar före</option><option value="10080">7 dagar före</option><option value="4320">3 dagar före</option><option value="2880">2 dagar före</option><option value="1440">1 dag före</option><option value="720">12 timmar före</option><option value="360">6 timmar före</option><option value="60">1 timme före</option></select></label><label>Svara senast<select name="responseDueRule" defaultValue="6h"><option value="0h">Vid aktivitetsstart</option><option value="1h">1 timme före</option><option value="2h">2 timmar före</option><option value="6h">6 timmar före</option><option value="previous-midnight">Dagen innan kl 00:00</option><option value="1d">1 dag före</option><option value="2d">2 dagar före</option><option value="3d">3 dagar före</option></select></label></div><label>Påminn obesvarade<select name="reminderMinutesBeforeDue" defaultValue="1440"><option value="0">Ingen påminnelse</option><option value="60">1 timme före svarstiden</option><option value="360">6 timmar före svarstiden</option><option value="720">12 timmar före svarstiden</option><option value="1440">1 dag före svarstiden</option><option value="2880">2 dagar före svarstiden</option></select></label></fieldset></>:null}
