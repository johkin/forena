...
export async function POST(request: Request) {
  ...
  if (seriesError || !series) {
    return NextResponse.json({ error: "Aktivitetsserien kunde inte sparas" }, { status: 500 });
  }

  const { data: activities, error: activitiesError } = await supabase.from("activities").insert(
    occurrences.map((item, index) => ({
      ...
      series_id: series.id,
      title,
      description_markdown: body.description?.trim() ?? "",
      gathering_at: item.gatheringAt,
